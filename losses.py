...
class TeacherAndSquareErrorDirichlet(tf.keras.losses.Loss):
    ...
    def __call__(self, y_true, y_pred, sample_weight=None):
        ...
        with K.name_scope(self._name_scope), graph_ctx:
            ...
            pred_prob = tf.cast(y_pred/tf.reduce_sum(y_pred, axis=-1, keepdims=True), dtype="float16")
            ...
            kld /= tf.cast(y_pred.shape[0], tf.float16) * tf.math.log(tf.cast(y_pred.shape[1], tf.float16))
            return losses + tf.cast(kld, tf.float32) * tf.minimum(self.epochs / 200, 1)
