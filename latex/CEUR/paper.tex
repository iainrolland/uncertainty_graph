...
% The "title" command
\title{Uncertainty-aware graph-based multimodal remote sensing detection of out-of-distribution samples}
...
In this paper, we have analyzed how well different measures of model uncertainty perform the task of identifying data points which belong to a distribution other than those observed during training (out of distribution detection).
...
The subjective opinions can be used to measure two intuitive measures of uncertainty: vacuity and dissonance.
Vacuity is a measure of the uncertainty related to an absence of observed evidence, i.e. a higher measure of vacuity suggests a lack of supporting evidence for a prediction.
Dissonance is a measure of prediction uncertainty arising due to the presence of conflicting evidence.
This approach (using graph-based neural networks within a subjective-logic framework) is, to the best of our knowledge, as-yet untested as a method for performing classification of multimodal remote sensing data.
The performance of the adopted technique represents a promising avenue in the search for meaningful uncertainty estimates for this task.
...
The proposed uncertainty-aware framework relies on the definition of uncertainty metrics, which in turn are based on subjective logic and a Dirichlet mapping \cite{ZhaoXujiang2020}.
These steps are detailed in this section, and have been properly adapted to the task of multimodal remote sensing classification.
...
An adaptation to the GCN architecture used by \cite{Kipf2017} must be made in order to obtain the subjective opinions that will be used to obtain measures of vacuity and dissonance uncertainty.
...
\end{equation}
which corresponds to the summation of Kullback-Leibler (KL) divergence terms between the teacher output probability and the expected value of the subjective model's Dirichlet distribution for each node.
Using $D_{\textrm{KL}}(\,\vert\vert\,)$ to compute the KL divergence, this is stated equivalently as $\sum_i\infdiv{\hat{p}_{ik}}{\mathbb{E}[p_{ik}]}$.
...
The `B' in the model names of Table \ref{loss_table} refers to the fact that dropout inference has been used as a Bayesian approximation.
...
\label{sec::results}
\subsection{Data}
A subsection of the 2018 IEEE GRSS Data Fusion Challenge dataset \cite{HoustonDataset} ws selected for the purposes of validating the described methods.
...
Models were implemented and trained using the TensorFlow library \cite{Abadi2015} on a personal laptop computer with Intel Core i7 CPU and 16 GB of RAM.
...
Of the types of uncertainty assessed, the measure of vacuity was the best metric to perform identification of OOD samples.
Experimental results have shown the performance of the S-BGCN-T-K model in the task of OOD detection to be improved against baseline methods.
